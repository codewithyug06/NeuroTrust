...
class MockAIService:
    ...
    def verify_news(url: str) -> dict:
        ...
        """
        time.sleep(0.8) # Network latency
        ...
        if is_safe:
            ...
            # Only populate triggers if risk is elevated (simulated)
            if intent_score > 0.2: 
                 psych_triggers = random.sample(triggers, k=random.randint(0, 1))

            # 3. AI Confidence Model
            ai_confidence = 0.85 + (abs(trust_score - 50) / 200) 
